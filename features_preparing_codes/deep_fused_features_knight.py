# -*- coding: utf-8 -*-
"""
Created on Sat Mar  5 19:38:47 2022
...
filet=pd.read_csv('C:\\Users\\mirazzak\\trainf0new.csv')
data='C:\\Users\\mirazzak\\data2\\data'

filev=pd.read_csv('C:\\Users\\mirazzak\\validf0new.csv')

#len(filev['task_2_label']==0)
#len(filev['task_2_label']==1)
...
class FuseModelDefault(torch.nn.Module):
    ...
    def forward(self,x,x1):
        ...
        global_features1=global_features.squeeze(4).squeeze(3).squeeze(2)
        print(global_features1.shape)
...
path='C:\\Users\\mirazzak\\3dmodeltask1.pth'
trained=torch.load(path)
model.load_state_dict(trained)
model.eval()
# inp=torch.rand(1,1,110,256,256)
# inp2=torch.rand(1,10)
# out=model(inp,inp2)

# #%% features to save
# data_features=np.array(features_train)
# data_featuresdf=pd.DataFrame(data_features)
# lst=[]
# for f in range(0,60):
#     ff='f_'+str(f)
#     lst.append(ff)
...
# print(pathfile['SubjectId'])
# new_list=lst
# old_columns=(data_featuresdf.columns)
# deepfeatures=data_featuresdf.copy()
# deepfeatures.rename(columns={old_columns[idx]: name for (idx,name) in enumerate(new_list)}, inplace=True)
# #firstdf.to_csv('A_FOS_features_train.csv',index=False)
# s=pathfile['SubjectId']
# deepfeatures.insert(0,'SubjectId',s)
# deepfeatures.to_csv('features_testdeep60.csv',index=False)
