...
class deeeeset(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        image =self._slice_extraction(image)
...
class ClassifierMLP(nn.Module):
    def __init__(self, in_ch: int, num_classes: Optional[int], layers_description: Sequence[int]=(256,), dropout_rate: float = 0.0):
        super().__init__()
        layer_list = []
...
class FuseModelDefault(torch.nn.Module):
    ...
    def __init__(self,fused_dropout_rate,num_classes,dropout_rate):
        ...
        self.conv_classifier_3d = nn.Sequential(nn.Conv3d(768, 256, kernel_size=1),
                                          nn.ReLU(),
...
###### new model for this challenge
import torch
from torch.utils.data.dataloader import DataLoader
# train_dataloader = DataLoader(dataset_train, batch_size=4,num_workers=6,shuffle=True)

# valid_dataloader = DataLoader(dataset_valid, batch_size=4,pin_memory=False,num_workers=6,shuffle=False)
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.net.head = nn.Linear(in_features=1024, out_features=512, bias=True)
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Linear(384, 2, bias=True)
        self.mlp=mlp
    ...
    def forward(self, x,x1):
        ...
        global_features = torch.cat((out1, features), dim=1)
